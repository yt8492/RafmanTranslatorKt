...
fun main() {
    window.addEventListener("load", {
        document.getElementsByTagName("textarea")
            .asList()
            .forEach { textarea ->
                if (textarea !is HTMLTextAreaElement) {
                    return@forEach
                }
                textarea.addEventListener("input",{
                    if (textarea.className == "input") {
                        val lines = textarea.value.count { it == '\n' } + 1
                        textarea.rows = maxOf(2, lines)
                        val translatedText = when (textarea.id) {
                            "RtJ_in" -> textarea.value.translateRafmanToJapanese()
                            "JtR_in" -> textarea.value.translateJapaneseToRafman()
                            else -> throw IllegalArgumentException("unknown textarea")
                        }
                        val outputTextArea = textarea.parentElement
                            ?.getElementsByClassName("output")
                            ?.get(0)
                                as? HTMLTextAreaElement
                            ?: throw IllegalStateException("output textarea not found")
                        with(outputTextArea) {
...
fun String.translateJapaneseToRafman(): String {
    val japaneseToRafmanMap = mapOf(
    ...
    return buildString {
        this@translateJapaneseToRafman.forEach {
            append(japaneseToRafmanMap["$it"] ?: "$it")
        }
    }
}

fun String.translateRafmanToJapanese(): String {
    val rafmanToJapaneseMap = mapOf(
    ...
    return buildString {
        this@translateRafmanToJapanese.forEach {
